...
class EmailSortingService:
    ...
    def sort_receipts(self):
        receipts_sorted = self.__sort_emails_with_subject_with_parent_label_to_sender(Subject.RECEIPT.value,
    ...
    def sort_job_postings(self):
        indeed_sorted = self.__sort_emails_from_with_parent_label(Email.INDEED.value, Label.JOB_POSTINGS.value,
                                                                  Label.INDEED.value, Color.BRIGHT_RED.value)
        ProgressBarUtil.update_progress(Label.INDEED.value + ' sorted: ', indeed_sorted)

        workopolis_sorted = self.__sort_emails_from_with_parent_label(Email.WORKOPOLIS.value, Label.JOB_POSTINGS.value,
                                                                      Label.WORKOPOLIS.value, Color.BRIGHT_RED.value)
        ProgressBarUtil.update_progress(Label.WORKOPOLIS.value + ' sorted: ', workopolis_sorted)

        neuvoo_sorted = self.__sort_emails_from_with_parent_label(Email.NEUVOO.value, Label.JOB_POSTINGS.value,
                                                                  Label.NEUVOO.value, Color.BRIGHT_RED.value)
        ProgressBarUtil.update_progress(Label.NEUVOO.value + ' sorted: ', neuvoo_sorted)

        glass_door_sorted = self.__sort_emails_from_with_parent_label(Email.GLASS_DOOR.value, Label.JOB_POSTINGS.value,
                                                                      Label.GLASS_DOOR.value, Color.BRIGHT_RED.value)
    ...
    def sort_online_orders(self):
        amazon_sorted = self.__sort_emails_from_with_parent_label(Email.AMAZON_DOMAIN.value, Label.ONLINE_ORDERS.value,
                                                                  Label.AMAZON.value, Color.AMAZON_ORANGE.value)
        ProgressBarUtil.update_progress(Label.AMAZON.value + ' sorted: ', amazon_sorted)

        intelcom_sorted = self.__sort_emails_from_with_parent_label(Email.INTELCOM_DOMAIN.value,
                                                                    Label.ONLINE_ORDERS.value, Label.INTELCOM.value,
                                                                    Color.AMAZON_ORANGE.value)
        ProgressBarUtil.update_progress(Label.INTELCOM.value + ' sorted: ', intelcom_sorted)

    def sort_rentals(self):
        rent_faster_sorted = self.__sort_emails_from_with_parent_label(Email.RENT_FASTER.value, Label.RENTALS.value,
                                                                       Label.RENT_FASTER.value, Color.ROYAL_BLUE.value)
        ProgressBarUtil.update_progress(Label.RENT_FASTER.value + ' sorted: ', rent_faster_sorted)

    def sort_google_emails(self):
        google_account_sorted = self.__sort_emails_from_with_parent_label(Email.GOOGLE_SECURITY_ALERTS.value,
                                                                          Label.GOOGLE_ALERTS.value,
                                                                          Label.GOOGLE_ACCOUNT.value,
                                                                          Color.ROYAL_BLUE.value)
    ...
    def __move_emails_from_sender_to_trash(self, from_email):
        emails = message_service.emails_from_inside_inbox(from_email)
    ...
    def __sort_emails_with_subject_to_working_label(self,
        ...
        emails_found = message_service.email_ids_with_criteria_inside_inbox(filter)
        ...
        if len(emails_found) > 0:
            ...
            if working_label is not None:
                return message_service.add_emails_to_labels_and_remove_from_inbox(emails_found, [working_label['id']])
        else:
            return []
    ...
    def __sort_emails_from_to_working_label(self,
        ...
        if len(emails_found) > 0:
            working_label = label_service.create_label_if_not_found(working_label_name, label_color)
            if working_label is not None:
                message_service.add_emails_to_labels_and_remove_from_inbox(emails_found, [working_label['id']])
        ...
        if parent_label is not None:
            ...
            if len(emails_found) > 0:
                ...
                for email_id in emails_found:
                    email = message_service.get_email(email_id)
                ...
                if len(email_dict) > 0:
                    for sender in email_dict:
                        found_label = label_service.create_label_if_not_found(sender,
                                                                              label_color)
                        message_service.add_emails_to_labels_and_remove_from_inbox(email_dict[sender],
                                                                                   [found_label['id']])
    ...
    def __sort_emails(self,
        ...
        if len(emails) > 0:
            ...
            for email in emails:
                email_id = email['id']
                message = message_service.get_email(email_id)
